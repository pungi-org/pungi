...
# Creating shims for those binaries will prevent pungi users to run those
...
conda_exists() {
  ...
  local condas=($(echo "${PUNGI_ROOT}/versions/"*"/bin/conda" "${PUNGI_ROOT}/versions/"*"/envs/"*"/bin/conda"))
...
# override `make_shims` to avoid conflict between pungi-virtualenv's `envs.bash`
# https://github.com/pyenv/pyenv-virtualenv/blob/v20160716/etc/pungi.d/rehash/envs.bash
make_shims() {
  local file shim
  for file do
    shim="${file##*/}"
